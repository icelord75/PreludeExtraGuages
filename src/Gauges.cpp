...
#include <Adafruit_ADS1X15.h>  // platformio lib install "Adafruit ADS1X15"
...
void DrawGauges()
{
        ...
        case STATE_OIL:
                ...
                if (OIL_TEMP < 40)
                        ...
                        TARGETPOS_R = 6; /* CENTER */
                else if (OIL_TEMP < 120)
                        TARGETPOS_R = 5;
                else if (OIL_TEMP < 130)
                        TARGETPOS_R = 4;
                else if (OIL_TEMP < 140)
                        TARGETPOS_R = 3;
                else if (OIL_TEMP < 150)
                        TARGETPOS_R = 2;
                else
                        TARGETPOS_R = 1; /* RED */
                break;
        ...
        // Arrage positions
        if (POSITION_L < TARGETPOS_L)
                POSITION_L++;
        if (POSITION_L > TARGETPOS_L)
                POSITION_L--;
        if (POSITION_R < TARGETPOS_R)
                POSITION_R++;
        if (POSITION_R > TARGETPOS_R)
                POSITION_R--;
        ...
        for (i = 0; i <= 43; i++)
        {
                ...
                if ((i >= 5 && i <= 25) && DRAW_L)
                { // LEFT 20
                        if ((TYPE_L == BAR) && (POSITION_L <= (i - 5)))
                                digitalWrite(SI_PIN, HIGH);
                        if ((TYPE_L == NEEDLE) && (POSITION_L == (i - 5)))
                                digitalWrite(SI_PIN, HIGH);
                }

                if ((i >= 31 && i <= 37) && DRAW_R)
                { // RIGHT 6
                        if ((TYPE_R == BAR) && (POSITION_R <= (i - 31)))
                                digitalWrite(SI_PIN, HIGH);
                        if ((TYPE_R == NEEDLE) && (POSITION_R == (i - 31)))
                                digitalWrite(SI_PIN, HIGH);
                }
                digitalWrite(SCK_PIN, HIGH);
                digitalWrite(SI_PIN, LOW);
